...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # inference equation: P(X|e) = alphaP(X,e)
        # use self.getObservationProb : returns probability of observation given Pacman's position, a potential ghost position, , and jail position
        # pacman's position: gameState.getPacmanPosition()
        # jail position: self.getJailPosition()
        # Start with large cloud of belief that shrinks over time
        # Beliefs: util.Counter object --> self.beliefs
        # only store self.beliefs
        
        # get packman's position
        pacmanPosition = gameState.getPacmanPosition()
        ...
        # for loop updating beliefs based on distance observation and pacman's position
        for p in self.allPositions:
            prob = self.getObservationProb(observation, pacmanPosition, p, jail)
            beliefs[p] = self.beliefs[p] * prob
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # assume ghosts move independently of aone another.
        # start with large cloud of belief
        beliefs = DiscreteDistribution()

        # for each previous ghost position
        for p in self.allPositions:
            # the distribution over new positions for the ghost
            newPosDist = self.getPositionDistribution(gameState, p)
            # for each position p in self.allPositions, newPostDist[p] is the probability that ghost is at position p at time t + 1
            for newPos, prob in newPosDist.items():

                beliefs[newPos] += self.beliefs[p] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # weight of a particle is the probability of the observation given Pacman’s position and that particle location.
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                prob *= self.getObservationProb(observation[i], pacman, p[i], self.getJailPosition(i))
            weights[p] += prob
