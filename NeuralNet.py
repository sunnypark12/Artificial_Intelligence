    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                next_iA.append(perceptron.sigmoidActivation(activation[-1]))
            activation.append(next_iA)
        return activation
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    delta = sum(nextLayer[k].weights[neuronNum + 1] * deltas[0][k] for k in range(len(nextLayer))) * gPrime
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    """
    while iteration < maxItr and weightChangeThreshold < weight:
    ...
    for input, output in examplesTest:
        if [round(n) for n in nnet.feedForward(input)[-1]] == output:
            testCorrect += 1
            continue
        testError += 1

    accuracy = testCorrect / (testCorrect + testError)

    print('Feed Forward Test correctly classified %d, incorrectly classified %d, test accuracy %f\n'%(testCorrect,testError,accuracy))
