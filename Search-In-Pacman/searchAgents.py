        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                ...
                if next_pos in self.corners:
                    corner_idx = self.corners.index(next_pos)
                    next_visited[corner_idx] = True
...
def cornersHeuristic(state, problem):
    ...
    position, visited_corners = state
    unvisited = [corner for i, corner in enumerate(corners) if not visited_corners[i]]
    if not unvisited:
        return 0

    dist = [manhattanDistance(position, corner) for corner in unvisited]
    min_dist = min(dist)
    near = unvisited[dist.index(min_dist)]
    val = min_dist
    curr = near
    unvisited.remove(near)

    while unvisited:
        dist = [util.manhattanDistance(curr, corner) for corner in unvisited]
        min_dist = min(dist)
        near = unvisited[dist.index(min_dist)]
        val += min_dist
        curr = near
...
def foodHeuristic(state, problem):
    ...
    min_dist = min(util.manhattanDistance(position, f) for f in food)
    ...
    for f in food:
        if f != s:
            heapq.heappush(edges, (util.manhattanDistance(s, f), s, f))

    while edges:
        cost, fromNode, toNode = heapq.heappop(edges)
        if toNode not in visited:
            visited.add(toNode)
            mst_cost += cost
            for f in food:
                if f not in visited:
                    heapq.heappush(edges, (util.manhattanDistance(toNode, f), toNode, f))
